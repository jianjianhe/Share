...
public class EditorControl {
    ...
    public Map<String,Object> uploadImage(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            String subCatalog   = new TimeUtil().getFormatDateForThree();
            String filePath = "";
            if("mp4".equals(fileExtension)){
                filePath = this.getClass().getResource("/").getPath().substring(1) + "/static/video/"+subCatalog+"/";
            }else{

                filePath = this.getClass().getResource("/").getPath().substring(1) + "/static/image/"+subCatalog+"/";
            }
            TimeUtil timeUtil = new TimeUtil();
            ...
            File f = fileUtil.multipartFileToFile(file, filePath, fileName);
            if("mp4".equals(fileExtension)){
                if("".equals(request.getServerPort())){
                    fileUrl = "http://"+request.getServerName()+"/video/"+subCatalog+"/"+f.getName();
                }else{
                    fileUrl = "http://"+request.getServerName()+":"+request.getServerPort()+"/video/"+subCatalog+"/"+f.getName();
                }
            }else{
//                fileUrl = fileUtil.uploadFile(fileUtil.multipartFileToFile(file, filePath, fileName), subCatalog);
                if("".equals(request.getServerPort())){
                    fileUrl = "http://"+request.getServerName()+"/image/"+subCatalog+"/"+f.getName();
                }else{
                    fileUrl = "http://"+request.getServerName()+":"+request.getServerPort()+"/image/"+subCatalog+"/"+f.getName();
                }
            }
